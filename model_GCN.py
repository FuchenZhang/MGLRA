...
class GCNLayer1(nn.Module):
    ...
    def forward(self, inputs, dia_len, topicLabel):
        ...
        if self.new_graph:
            # pdb.set_trace()
            adj = self.message_passing_directed_speaker(inputs, dia_len, topicLabel).to(device)
        else:
            adj = self.message_passing_wo_speaker(inputs, dia_len, topicLabel).to(device)
        x = torch.matmul(adj, inputs).to(device)
    ...
    def message_passing_wo_speaker(self, x, dia_len, topicLabel):
        ...
        adj_mask = torch.zeros((x.shape[0], x.shape[0]))
        start = 0

        def get_0_1_array(array, rate=0.15):
        ...
        #zfc = adj*get_0_1_array(np.random.randint(2, size=((x.shape[0], x.shape[0]))))
        adj_mask = adj * get_0_1_array(adj_mask)
    ...
    def message_passing_directed_speaker(self, x, dia_len, qmask):
        ...
        def get_0_1_array(array_, rate=0.2):
            '''按照数组模板生成对应的 0-1 矩阵，默认rate=0.2'''
            zeros_num = int(array_.shape[0] * array_.shape[1] * rate)  # 根据0的比率来得到 0的个数
            new_array = np.ones(array_.shape[0] * array_.shape[1])  # 生成与原来模板相同的矩阵，全为1
            new_array[:zeros_num] = 0  # 将一部分换为0
            np.random.shuffle(new_array)  # 将0和1的顺序打乱
