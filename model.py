...
class LSTMModel(nn.Module):
    ...
    def __init__(self, D_m, D_e, D_h, n_classes=7, dropout=0.5, att2=True):
        ...
        self.lstm1 = nn.LSTM(input_size=D_m, hidden_size=D_e, num_layers=2, bidirectional=True, dropout=dropout)
        self.lstm2 = nn.LSTM(input_size=D_m, hidden_size=D_e, num_layers=2, bidirectional=True, dropout=dropout)
        self.lstm3 = nn.LSTM(input_size=D_m, hidden_size=D_e, num_layers=2, bidirectional=True, dropout=dropout)
    ...
    def forward(self, textf, acouf, visuf, qmask, umask):
        ...
        # 加for循环
        for i in range(10):
            memory_textf, hidden1 = self.lstm1(textf[:, :, :100])
            memory_acouf,hidden2 = self.lstm2(acouf[:, :, :100])
            memory_visuf, hidden3 = self.lstm3(visuf[:, :, 100])
            memory = memory_visuf + memory_textf + memory_acouf
        # emotions, hidden = self.lstm(U)
        #emotions, hidden = self.lstm1(U)
        #emotions, hidden = self.lstm2(A)
        #emotions, hidden = self.lstm3(V)
        ...
        if self.att2:
            ...
            alpha = []
            for t in memory:
                att_em, alpha_ = self.matchatt(memory, t, mask=umask)
                att_emotions.append(att_em.unsqueeze(0))
                alpha.append(alpha_[:, 0, :])
            att_emotions = torch.cat(att_emotions, dim=0)
            hidden = F.relu(self.linear(att_emotions))
        else:
            hidden = F.relu(self.linear(memory))

        hidden = self.dropout(hidden)
        log_prob = F.log_softmax(self.smax_fc(hidden), 2)
        return log_prob, alpha, alpha_f, alpha_b, memory
...
class DialogueGNNModel(nn.Module):
    ...
    def __init__(self, base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, n_speakers, max_seq_len, window_past, window_future, n_classes=7,
                 ...
                 dynamic_edge_w=False, D_m_v=256, D_m_a=100, modals='avl', att_type='gated', av_using_lstm=False, Deep_GCN_nlayers=64, dataset='IEMOCAP',
        ...
        print('DialogueGNNModel is using! --------------------------------------')
        ...
        if self.base_model == 'DialogRNN':
            ...
            if not self.multi_modal:
                ...
                if 'v' in self.modals:
                                            #   342    200
                    self.linear_v = nn.Linear(256, hidden_v)
                    if self.av_using_lstm:
                ...
                self.rnn_parties = nn.GRU(input_size=200, hidden_size=D_e, num_layers=2, bidirectional=True, dropout=dropout)
                self.rnn_parties2 = nn.GRU(input_size=100, hidden_size=D_e, num_layers=2, bidirectional=True,dropout=dropout)
            ...
            if not self.multi_modal:
                ...
                hidden_a = 200
                hidden_v = 200
                hidden_l = 200                                    # 1582    200
                if 'a' in self.modals: self.linear_a = nn.Linear(D_m_a, hidden_a)
                if 'v' in self.modals: self.linear_v = nn.Linear(256, hidden_v)
        ...
        if self.multi_modal:
            ...
            if self.att_type == 'concat_subsequently':
                self.smax_fc = nn.Linear(400, n_classes) if self.use_residue else nn.Linear(400, n_classes)
    ...
    def forward(self, U, qmask, umask, seq_lengths, U_a=None, U_v=None, test_label=False):
        ...
        if self.base_model == "DialogRNN":
            ...
            if not self.multi_modal:
                ...
                if 'a' in self.modals:
                    ...
                    if self.use_crn_speaker:
                        ...
                        E_parties_ = [self.rnn_parties2(U_parties_[p].transpose(0, 1))[0].transpose(0, 1) for p in range(len(U_parties_))]
            ...
            emotions_m = emotions_l + emotions_v + emotions_a
            # print(emotions_m.size())
        ...
        if not self.multi_modal:
            ...
            if 'a' in self.modals:
                if self.graph_type == 'relation':
                    features_a, edge_index, edge_norm, edge_type, edge_index_lengths = batch_graphify(emotions_m, qmask, seq_lengths, self.window_past,
                                                                                                      self.window_future, self.edge_type_mapping,
                                                                                                      self.att_model, self.no_cuda)
                else:
                    features_a, edge_index, edge_norm, edge_type, edge_index_lengths = simple_batch_graphify(emotions_m, seq_lengths, self.no_cuda)
            else:
                features_a = []
            # if 'v' in self.modals:
            #     if self.graph_type == 'relation':
        ...
        if self.graph_type == 'relation':
            if not self.multi_modal:
                ...
                if 'a' in self.modals:
                    ...
                    print("emotion_a", emotions_a.size())
                else:
                    emotions_a = []
                # if 'v' in self.modals:
                ...
                # size, dim = emotions_feat.shape
                # emotions_feat = emotions_feat.view(-1, 31, 400)
                print("size", emotions_feat.size())
